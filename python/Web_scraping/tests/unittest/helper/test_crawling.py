...
class MyTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.site_selectors = {
            helper.crawling.Crawling.URLS_TARGET: [
                (By.XPATH,
                 '//*[@id="banners"]/a/img',
        ...
        self.crawling_items = {helper.crawling.Crawling.URLS_TARGET: [], helper.crawling.Crawling.URLS_EXCLUSION: []}
    ...
    def test___init___01(self):
        ...
        with self.assertRaisesRegex(ValueError, '^Crawling.test___init___01引数エラー:value_object=None'):
            test_target = helper.crawling.Crawling()

    def test___init___02(self):
        """引数無コンストラクタ"""
        with self.assertRaisesRegex(ValueError, '^Crawling.test___init___02引数エラー:site_selectors=None'):
            test_target = helper.crawling.Crawling(self.site_url)

    def test___init___03(self):
        """引数無コンストラクタ"""
        test_target = helper.crawling.Crawling(self.site_url, self.site_selectors)
        self.assertTrue(isinstance(test_target, helper.crawling.Crawling))
        self.assertNotEqual(helper.crawling.Crawling.value_object, test_target.value_object)
        self.assertEqual(test_target.get_crawling_file_path(), test_target.value_object.crawling_file_path)

    def test___init___04(self):
        """引数無コンストラクタ"""
        test_target = helper.crawling.Crawling(self.site_url, self.site_selectors, self.crawling_items, self.crawling_file_path)
        self.assertTrue(isinstance(test_target, helper.crawling.Crawling))
        self.assertNotEqual(helper.crawling.Crawling.value_object, test_target.value_object)

    def test_get_value_object_01(self):
        test_target = helper.crawling.Crawling(self.site_url, self.site_selectors)
        self.assertTrue(isinstance(test_target.get_value_object(), helper.crawling.CrawlingValue))
    ...
    def test_get_crawling_file_path_01(self):
        ...
        self.assertEqual(helper.crawling.Crawling.crawling_file_path, test_target.get_crawling_file_path())
